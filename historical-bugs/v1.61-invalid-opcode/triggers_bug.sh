...
main() {
    ...
    BIN1="out_${RUSTC_V1}_O${OPT1}"
    BIN2="out_${RUSTC_V2}_O${OPT2}"

    echo "Compiling the two files... $BIN1 and $BIN2"
    RUSTUP_TOOLCHAIN=$RUSTC_V1 $RUSTC -C opt-level=$OPT1 bug.rs -o $BIN1
    RUSTUP_TOOLCHAIN=$RUSTC_V2 $RUSTC -C opt-level=$OPT2 bug.rs -o $BIN2
